...
const DonorPage = () => {
  ...
  const approvedDonations = donationHistory.filter(d => d.status === 'approved');

  const donationCountsByYear = approvedDonations.reduce((acc, donation) => {
  ...
  const handleFirstDonation = async e => {
    ...
    try {
      ...
      toast.success("Donation request submitted. Waiting for admin approval.");
        ...
        <Row id="status" style={{ marginTop: "120px", marginBottom: "60px" }}>
          <Col md={6}>
            <div className="donor-status-panel glass-effect shadow-lg p-4 rounded">
              ...
              <div className="mt-4">
                {donationHistory.some(d => d.status === 'pending') && (
                  <div className="alert alert-warning mt-3">
                    ⏳ You have a pending donation request. Please wait for admin approval.
                  </div>
                )}

                <Button
                  variant="success"
                  disabled={donationHistory.some(d => d.status === 'pending')} // ✅
                  className="w-100"
