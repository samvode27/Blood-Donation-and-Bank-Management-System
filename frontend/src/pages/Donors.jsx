...
const Donors = () => {
  ...
  const handleApprove = async (donorId, donationId) => {
    try {
      const res = await publicRequest.put(`/donors/donation/approve`, {
        donorId,
        donationId,
      });
      ...
      // Refresh donors OR update UI properly
      fetchDonors(); // best option
    } catch (err) {
      console.error(err);
  ...
  const handleReject = async (donorId, donationId) => {
    try {
      const res = await publicRequest.put(`/donors/donation/reject`, {
        donorId,
        donationId,
      });
             ...
             <td data-label="Status">
                <span
                  className={`status-badge ${
                    donor.donationHistory?.[0]?.status || "pending"
                  }`}
                >
                  {donor.donationHistory?.length > 0
                    ? donor.donationHistory[0].status
                    : "pending"}
                </span>
              </td>
              ...
              <td className="action-buttons">
                {donor.donationHistory?.map((donation) => (
                  <div key={donation._id} className="donation-actions">
                ...
                {/* Existing buttons */}
                <button onClick={() => handleEditClick(donor)}>
                  <Pencil size={16} />
                </button>

                <button onClick={() => confirmDelete(donor)} className="delete">
                  <Trash size={16} />
                </button>
              </td>
            </tr>
