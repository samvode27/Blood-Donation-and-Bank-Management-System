...
const donateBlood = async (req, res) => {
   ...
   try {
      ...
      if (age < 18 || weight < 50) {
         return res.status(400).json({
            success: false,
            message: 'Not eligible (age >=18 and weight >=50 required)'
         });
      }

      // ✅ Add as PENDING (not approved yet)
      ...
      res.status(201).json({
         success: true,
         message: 'Donation request submitted. Waiting for admin approval.'
      });
...
const approveDonation = async (req, res) => {
   ...
   try {
      ...
      donor.nextDonationDate = new Date(donation.date.getTime() + 56 * 24 * 60 * 60 * 1000);

      await donor.save();

      res.status(200).json({ message: "Donation approved successfully", donor });
...
const rejectDonation = async (req, res) => {
   ...
   try {
      ...
      res.status(200).json({ message: "Donation rejected", donor });
...
module.exports = {
   deleteDonor, getOneDonor, getAllDonor, getDonorStats, updateDonor, createDonor, signin, signup, rejectDonation,   
   sendVerificationCode, verifyVerificationCode, changePassword, sendForgotPasswordCode, verifyForgotPasswordCode,
   getBloodGroupStats, getMyProfile, updateProfile, getTopDonors, getDonationHistory, donateBlood, getDonationsPerYear, approveDonation
}